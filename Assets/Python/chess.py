...
class Board(BaseBoard):
    ...
    def generate_pseudo_legal_ep(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        if self.turn:
            rank_mask = BB_RANKS[4]
        else:
            rank_mask = BB_RANKS[3]
    ...
    def push(self, move):
        ...
        if castling:
            ...
            if a_side:
                ...
                if self.turn == WHITE:
                    self._set_piece_at(C1, KING, self.turn)
                else:
                    self._set_piece_at(C8, KING, self.turn)
                # self._set_piece_at(D1 if self.turn == WHITE else D8, ROOK, self.turn)
                if self.turn == WHITE:
                    self._set_piece_at(D1, ROOK, self.turn)
                else:
                    self._set_piece_at(D8, ROOK, self.turn)
            else:
                # self._set_piece_at(G1 if self.turn == WHITE else G8, KING, self.turn)
                if self.turn == WHITE:
                    self._set_piece_at(G1, KING, self.turn)
                else:
                    self._set_piece_at(G8, KING, self.turn)
                # self._set_piece_at(F1 if self.turn == WHITE else F8, ROOK, self.turn)
                if self.turn == WHITE:
                    self._set_piece_at(F1, ROOK, self.turn)
                else:
                    self._set_piece_at(F8, ROOK, self.turn)
    ...
    def _epd_operations(self, operations):
        ...
        for opcode, operand in operations.items():
            ...
            if hasattr(operand, "__iter__"):
                ...
                if opcode == "pv":
                    position = Board(self.shredder_fen())
                else:
                    position = self
    ...
    def epd(self, shredder=False, en_passant="legal", promoted=None, **operations):
        ...
        *hmvc* and *fmvc* are not included by default. You can use:
        ...
        if shredder:
            epd.append(self.castling_shredder_fen())
        else:
        ...
        if en_passant == "fen":
            ...
            if self.has_pseudo_legal_en_passant():
                epd.append(SQUARE_NAMES[self.ep_square])
            else:
                epd.append("-")
        else:
    ...
    def _transposition_key(self):
        ...
        if self.has_legal_en_passant():
            return (self.pawns, self.knights, self.bishops, self.rooks,
                self.queens, self.kings,
                self.occupied_co[WHITE], self.occupied_co[BLACK], self.promoted,
                self.turn, self.clean_castling_rights(), self.ep_square)
        else:
            return (self.pawns, self.knights, self.bishops, self.rooks,
                self.queens, self.kings,
                self.occupied_co[WHITE], self.occupied_co[BLACK], self.promoted,
                self.turn, self.clean_castling_rights(), None)
